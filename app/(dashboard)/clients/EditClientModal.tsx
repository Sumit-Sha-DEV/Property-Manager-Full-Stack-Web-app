...
export function EditClientModal({ client, isOpen, onClose }: { client: any, isOpen: boolean, onClose: () => void }) {
  ...
  const [budget, setBudget] = useState<string>(client?.budget?.toString() || '')

  // Requirement Classification States
  const [propertyType, setPropertyType] = useState<typeof PROPERTY_TYPES[number]>(client?.configuration?.property_type || 'Residential')
  const [category, setCategory] = useState<string>(client?.configuration?.category || CATEGORIES.Residential[0])
  const [requiredArea, setRequiredArea] = useState<string>(client?.configuration?.required_area || '')
  const [selectedBHK, setSelectedBHK] = useState<string[]>(client?.configuration?.bhk || [])
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Edit Client Profile">
      <form onSubmit={handleSubmit} className="space-y-5 max-h-[80vh] overflow-y-auto px-1 pb-4">
        ...
        <div className="bg-slate-50/80 p-4 rounded-2xl border border-slate-100 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input name="name" defaultValue={client?.name} required className="w-full px-3 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:outline-none bg-white text-sm" />
            </div>
            <div>
              <label className="block text-[11px] font-bold text-slate-700 mb-1 tracking-wide">Mobile Number</label>
              <input name="phone" defaultValue={client?.phone} type="tel" required className="w-full px-3 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:outline-none bg-white text-sm" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-indigo-50/30 p-4 rounded-2xl border border-indigo-100/50 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select name="requirement" defaultValue={client?.requirement} className="w-full px-3 py-2 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:outline-none bg-white text-sm font-medium">
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea name="notes" defaultValue={client?.notes} rows={3} className="w-full px-4 py-2.5 border rounded-xl focus:ring-2 focus:ring-indigo-500/20 focus:outline-none bg-white text-sm"></textarea>
          </div>
        </div>
